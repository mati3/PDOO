...
public class ControladorQytetet {
        ...
        while(!termino){
            ...
            if (!jugador.getEncarcelado()){
                ...
                if(casilla.getTipo() == TipoCasilla.CALLE){
                    ...
                    if(((Calle)casilla).tengoPropietario() && ((Calle)casilla).getTituloPropiedad().getNombre()!=this.jugador.getNombre()){  
                        vista.mostrar(" Tengo que pagar alquiler " + ((Calle)casilla).getTituloPropiedad().getNombre() + " es el propietario \n " );
    ...
    public Calle ElegirCasillaDePropiedad (boolean hipotecada, boolean todo){// hipotecada si entra true mostrare todas, si entra false mostraré solo las NO hipotecadas
        ...
        if(hipotecada == true){
            ...
            if(todo){ // muestra solo las SI hipotecadas cuando todo esta a true
                for(int i = 0; i < jugador.getPropiedades().size();i++){
                    aux = jugador.getPropiedades().get(i).getCasilla();   
                    if(((Calle)aux).getTituloPropiedad().getHipotecada()){
                        casillas.add(aux);
                    }
                }
            }else{
