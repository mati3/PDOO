...
module ModeloQytetet
  ...
  class Qytetet
    ...
    #EXAMEN ###################################
    # si el jugador que recibe la donacion esta en la carcel, no se podrá donar
    # si se dona, y las casillas tiene casas y hoteles, estas se las quedan 
    # el jugador actual en una propiedad al azar.
    def donarPropiedad(namejugador,casita) # boolean donacion
      donacion = @jugadorActual.encarcelado
        if (!donacion)
          @jugadores.each do |aux|
            if(aux.nombre == namejugador)
              aux.propiedades << casita.titulo # el jugador que entra es una copia, por eso lo busco aqui
            end
          end
          @jugadorActual.propiedades.delete(casita.titulo)
          if(@jugadorActual.tengoPropiedades )
            if casita.numCasas > 0 # entiendo que podria superar la restriccion de 4 casas
              @jugadorActual.propiedades[rand(@jugadorActual.propiedades.size)].getCasilla.numCasas += casita.numCasas #buscar metodos
            end
            if casita.numHoteles > 0
              @jugadorActual.propiedades[rand(@jugadorActual.propiedades.size)].getCasilla.numHoteles += casita.numHoteles
            end
          end
        end
      return !donacion # si está encarcelado NO se puede donar, se devuelve lo contrario
    end
    #FIN EXAMEN###############################
  end
