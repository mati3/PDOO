...
module InterfazTextualQytetet
  class ControladorQytetet
    ...
    def inicializacionJuego # void
      jugadores = @vista.obtener_nombre_jugadores # hago jugadores una variable global para usarlo en examen # esto es un array de string, de nombres
        ...
        if (!@jugador.encarcelado)
          ...
          if(@casilla.tipo == TipoCasilla::CALLE)
            ...
            if(@casilla.tengoPropietario && @casilla.titulo.nombre != @jugador.nombre)
              ...
              @vista.mostrar("Jugador "+ @jugador.nombre + " Tiene un SaldoNuevo " + @jugador.saldo.to_s)
              ...
              if(@vista.elegir_quiero_comprar)
                ...
                @vista.mostrar("Jugador " + @jugador.nombre + " Ha comprado el Titulo de Propiedad " + @casilla.titulo.nombre + " compruebo " + neox.to_s)
               # @jugador.propiedades.each do |a|
                #  @vista.mostrar(" nombre jugador " + @jugador.nombre)
            ...
            if(@casilla.tipo == TipoCasilla::SORPRESA)#es de tipo sorpres
              ...
              @vista.mostrar("Jugador " + @jugador.nombre + " Esta en una casilla SORPRESA, He aplicado mi sorpresa CartaActual: \n " + @juego.cartaActual.to_s)
            end
            @vista.pausar
          end
     #////// tengo propiedades, no estoy en bancarrota y no estoy en la carcel, GESTION INMOBILIARIA //////////////////
          if(@jugador.saldo > 0 && @jugador.tengoPropiedades)
            ...
            when 5 # muestro solo las SI hipotecadas
              ...
              @juego.propiedadesHipotecadasJugador(nil).each do |aux|
                @vista.mostrar(aux.to_s + "\n ")
              end
            ...
            # EXAMEN ************************
            when 6 # donar una propiedad a otro jugador
              @vista.mostrar(" lista de propiedades 6")
              @jugador.propiedades.each do |aux|
                @vista.mostrar(aux.to_s + "\n ")
              end
              auxjugadores=Array.new
              @juego.jugadores.each do |aux|  
                if(aux != @jugador)
                  auxjugadores << aux.nombre
                end
              end
              posJugador = @vista.eligeJugador(auxjugadores) # esto devuelve un entero
              nombreJugador = auxjugadores[posJugador]
              @casilla = ElegirCasillaDePropiedad(false)
              @vista.mostrar(" si el jugador elegido esta en la carcel no logro donar ")
              conseguido = @juego.donarPropiedad(nombreJugador,@casilla)
              @vista.mostrar("  He logrado donar la casilla: " + conseguido.to_s)
            # FIN EXAMEN *********************************
            else
