...
module InterfazTextualQytetet
  ...
  def menu_gestion_inmobiliaria
    ...
    menuGI=[[0, 'Siguiente Jugador'], [1, 'Edificar casa'], [2, 'Edificar Hotel'], [3, 'Vender propiedad'],[4, 'Hipotecar Propiedad'], [5, 'Cancelar Hipoteca'],[6, 'Donar Propiedad']]
  ...
  def elegir_quiero_comprar
    mostrar( 'Elige la opcion para comprar una calle')
    menuSC=[[0, 'No quiero comprar'], [1, 'Si quiero comprar']]
    salida=self.seleccion_menu(menuSC)
    mostrar( 'has elegido')
    mostrar(salida)
    if(salida == 0)
...
def menu_elegir_propiedad(listaPropiedades) # numero y nombre de propiedades            
  ...
  def pausar
     self.mostrar("Pulsa ENTER para cotinuar:")
     lectura = gets.chomp
  end
